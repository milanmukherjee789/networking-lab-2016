...
/* All of the following deals with command line switches */
  while ((opt = getopt (argc, argv, "dnoh:P:p:g:l:w:")) != -1)	/* this function is handy */
	...
	case 'g':		/*get a file from the server */
	  printf("File ,going to be downloaded -> %s\n",optarg );
	  strncpy (filename, optarg, sizeof (filename) - 1);
	  opcode = RRQ;
      ...
      /* if(debug)
    	...
    	if (debug)
  		...
  		while (1)			/* our break statement will escape us when we are done */{
     		...
     		if (debug){
	  			printf("The first data block has been read from the file and will be sent to the server\n");
      		...
      		if (sendto(sock, packetbuf[bcount], len, 0, (struct sockaddr *) &server,
	   															sizeof (server)) != len){
	  			if (debug)
      		...
      		if (((count) % ackfreq) == 0 || ssize != datasize){
	  			...
	  			for (j = 0; j < RETRIES; j++){
	      			...
	      			n = -1;
	     			for (i = 0; errno == EAGAIN && i <= TIMEOUT && n < 0; i++){
		  				n = recvfrom (sock, recvbuf, sizeof (recvbuf), MSG_DONTWAIT,(struct sockaddr *) &ack,
	      			...
	      			else{		/* checks to ensure send to ip is same from ACK IP */
		  				if (server.sin_addr.s_addr != ack.sin_addr.s_addr){
		      				if (debug)
								printf("Client: Error recieving ACK (ACK from invalid address)\n");
		  				...
		  				if (tid != ntohs (server.sin_port)){
		      				if (debug)
								printf("Client: Error recieving file (data from invalid tid)\n");
		      					len = err_packet (5, err_msg[5], buf);
		      					/* send the data packet */
		     					if (sendto(sock, buf, len, 0, (struct sockaddr *) &server,
			   															sizeof (server)) != len){
			 						printf("Client: Mismatch in number of sent bytes while trying to send mode error packet\n");
								}
		      					/*if (debug)
...
void tget (char *pFilename, struct sockaddr_in server, char *pMode, int sock){
  /* local variables */
	...
	if (debug)
  	...
  	if (fp == NULL){				//if the pointer is null then the file can't be opened - Bad perms 
    	if (debug)
...
/* zero the buffer before we begin */
	...
	n = datasize + 4;          // each DATA packet is 4 + 512 
	do{
      	/* zero buffers so if there are any errors only NULLs will be exposed */
    	memset (packetbuf, 0, sizeof (packetbuf));
      	memset (ackbuf, 0, sizeof (ackbuf));
      	if (n != (datasize + 4)){          // Only is the packet is last n<512+4
	  		if (debug)
    	...
    	for (j = 0; j < RETRIES; j++){	/* this allows us to loop until we either break out by getting the correct ack OR time out because we've looped more than RETRIES times */
			server_len = sizeof (data);
		  	...
		  	n = -1;
		  	for (i = 0; errno == EAGAIN && i <= TIMEOUT && n < 0; i++){  // till the timeout && error and recvfrom()_error
		    	n = recvfrom (sock, packetbuf, sizeof (packetbuf) - 1,MSG_DONTWAIT, 
		    		(struct sockaddr *) &data,(socklen_t *) & server_len);
		      	usleep (1000);
		    }
		  	if (!tid){
		      ...
		      //resend packet
		    	...
		    	if (server.sin_addr.s_addr != data.sin_addr.s_addr){
			  		...
			  		j--;      // discard all those above tasks and perform from beginning
			  		continue;
				}
	...
	/*  All is well and proceed for writing into file descriptor fp */
		    	bufindex = (char *) packetbuf;	//start our pointer going
		      	...
		      	if (opcode != 3){	/* packetbuf is not data packet some corrupted content */
			  		if (debug)
			    		printf("Badly ordered/invalid data packet (Got OP: %d Block: %d) (Wanted Op: 3 Block: %d)\n",
			       				opcode, rcount, count);
	/* sending error message */
			  		if (opcode > 5){
			      		len = sprintf ((char *) packetbuf,"%c%c%c%cIllegal operation%c",
